...
function Resolve-RepoConnJson {
  param([string]$Root)

  $matches = @(Get-ChildItem -Path $Root -Recurse -Force -File -Filter conn.json |
  ...
  if ($matches.Count -gt 1) {
    throw "Multiple .mcs/conn.json files found. Narrow this script for your target project. Found: $($matches -join '; ')"
  }

  throw "No .mcs/conn.json found under $Root"
}

$connPath = Resolve-RepoConnJson -Root $repoRoot
