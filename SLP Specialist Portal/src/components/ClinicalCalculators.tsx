...
export function ClinicalCalculators() {
  ...
  const getRange = (score: any) => {
    if (selectedId === 'moca') return getMocaInterpretation(score.value);
    if (selectedId === 'masa') return getMasaInterpretation(score.value);
    if (selectedId === 'eat-10') return getEat10Interpretation(score.value);
    if (selectedId === 'guss') return getGussInterpretation(score.value);
    if (selectedId === 'flci') return getFlciInterpretation(score.value);
    if (selectedId === 'ripa-g2') return getRipaG2Interpretation(score.value);
    if (selectedId === 'wab-r') return getWabrInterpretation(aqScore.value);
    if (selectedId === 'aids') return getAidsInterpretation(aidsScore.value);
    if (selectedId === 'fda-2') return getFda2Interpretation(score.value);
    return null;
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
              <div className="bg-slate-900 p-8 rounded-[2rem] text-white flex flex-col justify-center items-center">
                ...
                <div className="text-6xl font-black tracking-tighter text-teal-400">
                  {selectedId === 'wab-r' ? aqScore.value.toFixed(1) : selectedId === 'aids' ? `${aidsScore.value}%` : totalScore.value}
                </div>
