param(
    ...
    [string]$OutputPath = 'D:\SNF AI Dashboard\data\processed\command_center_automated_handoff_dispatch_log.csv',
    [string]$SummaryPath = 'D:\SNF AI Dashboard\data\processed\command_center_automated_handoff_dispatch_summary.md',
    [string]$DeliveryMode = '',
    [string]$WebhookUrl = '',
...
if ([string]::IsNullOrWhiteSpace($effectiveDeliveryMode)) {
    $effectiveDeliveryMode = Get-DotEnvValue -Path 'D:\SNF AI Dashboard\.env' -Key 'HANDOFF_DELIVERY_MODE'
}
if ([string]::IsNullOrWhiteSpace($effectiveDeliveryMode)) {
...
if ([string]::IsNullOrWhiteSpace($effectiveWebhookUrl) -and $effectiveDeliveryMode -ne 'CopilotNative') {
    ...
    if ([string]::IsNullOrWhiteSpace($candidateHandoff)) {
        $candidateHandoff = Get-DotEnvValue -Path 'D:\SNF AI Dashboard\.env' -Key 'HANDOFF_WEBHOOK_URL'
    }
    $candidateTeams = [string]$env:HANDOFF_TEAMS_WEBHOOK_URL
    if ([string]::IsNullOrWhiteSpace($candidateTeams)) {
        $candidateTeams = Get-DotEnvValue -Path 'D:\SNF AI Dashboard\.env' -Key 'HANDOFF_TEAMS_WEBHOOK_URL'
    }
...
if ([string]::IsNullOrWhiteSpace($effectiveBearerToken)) {
    $effectiveBearerToken = Get-DotEnvValue -Path 'D:\SNF AI Dashboard\.env' -Key 'HANDOFF_WEBHOOK_BEARER_TOKEN'
}
    ...
    elseif (-not [string]::IsNullOrWhiteSpace($effectiveWebhookUrl)) {
        try {
            $body = @{
                ...
                groundednessStatus = [string]$row.GroundednessStatus
                verificationStatus = [string]$row.VerificationStatus
                uncertaintyReason = [string]$row.UncertaintyReason
