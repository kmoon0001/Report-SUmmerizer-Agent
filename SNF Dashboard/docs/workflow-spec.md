...
- Clinical purpose: creates a shareable care summary for approved internal or external use.
- Trigger: called from assistant, dashboard export request, discharge workflow, or family communication workflow.
- Inputs: patientId, episodeOfCare, exportType, role, audience, dateRange, includeEducation, includeGoals, consentFlags.
- Outputs: status, fileUrl, pdfUrl, recordKey, sentAt, disclosureFlags.
- One-sentence clinical purpose: packages a patient care summary in a controlled format for clinical handoff, review, or family communication.
- Step-by-step logic:
  1. Validate the requestor role, audience, and consent status.
  2. Retrieve current problem list, goals, interventions, progress, and pending tasks.
  3. Exclude restricted data based on consent, role, and minimum necessary rules.
  4. Generate the summary in the requested format.
  5. Label and store the export according to sensitivity and retention rules.
- Clinical validation steps:
  1. Confirm data is current for the requested date range.
  2. Verify care plan and medication sections reflect the latest reconciled state.
  3. Check disclosure appropriateness against consent and privacy policy.

## 5. CreateClinicalDocument
...
## 8. InitiateFallResponseProtocol
...
## 9. InitializeInfectionControlProtocol
...
## 10. GenerateQualityMeasureReport
