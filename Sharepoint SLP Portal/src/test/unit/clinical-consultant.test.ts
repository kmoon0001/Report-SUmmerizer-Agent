...
import { clinicalConsultant } from "../../services/clinical-consultant";
...
describe("ClinicalConsultantService", () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("consults the AI service with enhanced context", async () => {
    ...
    const result = await clinicalConsultant.consult(
      query,
      context,
      [],
      "Dysphagia",
    ...
    expect(aiService.generateClinicalResponse).toHaveBeenCalledWith(
      ...
      expect.stringContaining(
        "[ACTIVE CLINICAL PATHWAY CONTEXT]: Dysphagia Pathway",
      ),
      [],
      "Dysphagia",
    );
  ...
  it("consults the AI service without context", async () => {
    ...
    const result = await clinicalConsultant.consult(query, "", [], "General");
  ...
  it("handles errors and logs them", async () => {
    ...
    await expect(
      clinicalConsultant.consult(query, "", [], "General"),
    ).rejects.toThrow(
      "Failed to get clinical guidance from the AI assistant. Please try again.",
    );

    expect(logger.error).toHaveBeenCalledWith(
      "ClinicalConsultantService.consult error:",
      error,
    );
  });
});
