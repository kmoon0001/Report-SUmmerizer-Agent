/**
 * OT Evidence-Based Treatment Protocols Tests
 */
...
} from "../../data/ot-evidence-based-treatment-protocols";

describe("OT Evidence-Based Treatment Protocols", () => {
  it("should have 5 treatment protocols", () => {
    expect(getAllProtocols().length).toBe(5);
  });

  it("should return protocol by ID", () => {
    const protocol = getProtocolById("ot-tp-001");
    expect(protocol?.condition).toContain("Occupational Performance");
  });
  ...
  it("should get protocol phases", () => {
    const phases = getProtocolPhases("ot-tp-001");
