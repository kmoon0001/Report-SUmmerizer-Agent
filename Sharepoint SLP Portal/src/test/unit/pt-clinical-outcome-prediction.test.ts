/**
 * PT Clinical Outcome Prediction Tests
 */
...
} from "../../data/pt-clinical-outcome-prediction";

describe("PT Clinical Outcome Prediction", () => {
  it("should have 5 outcome predictors", () => {
    expect(getAllOutcomePredictors().length).toBe(5);
  });

  it("should return predictor by ID", () => {
    const predictor = getOutcomePredictorById("pt-op-001");
    expect(predictor?.condition).toContain("Stroke");
  });
  ...
  it("should predict functional outcome", () => {
    const prediction = predictFunctionalOutcome("pt-op-001", [
      "FMA >50 at admission",
      "Age <65 years",
    ]);
