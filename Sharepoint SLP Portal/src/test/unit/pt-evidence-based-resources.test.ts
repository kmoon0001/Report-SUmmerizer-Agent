/**
 * PT Evidence-Based Resources & Clinical Depth Tests
 */

import { describe, it, expect } from "vitest";
import {
  getPTEvidenceResourceById,
  getAllPTEvidenceResources,
  getPTEvidenceResourcesByCategory,
  searchPTEvidenceResources,
  getPTEvidenceResourcesByEvidenceLevel,
  getKeyFindings,
  validateClinicalApplication,
} from "../../data/pt-evidence-based-resources";

describe("PT Evidence-Based Resources & Clinical Depth", () => {
  it("should have 5 evidence-based resources", () => {
    expect(getAllPTEvidenceResources().length).toBe(5);
  });

  it("should return resource by ID", () => {
    const resource = getPTEvidenceResourceById("pt-ebr-001");
    expect(resource?.name).toContain("Strength Training");
  });

  it("should return undefined for invalid ID", () => {
    expect(getPTEvidenceResourceById("invalid")).toBeUndefined();
  });

  it("should filter by category", () => {
    const resources = getPTEvidenceResourcesByCategory("Neurological");
    expect(resources.length).toBeGreaterThan(0);
  });

  it("should search resources", () => {
    const results = searchPTEvidenceResources("Stroke");
    expect(results.length).toBeGreaterThan(0);
  });

  it("should filter by evidence level", () => {
    const resources = getPTEvidenceResourcesByEvidenceLevel(1);
    expect(resources.length).toBeGreaterThan(0);
  });
  ...
  it("should reject invalid applications", () => {
    const result = validateClinicalApplication("pt-ebr-001", "Invalid");
    expect(result.valid).toBe(false);
  });

  it("should have consistent data", () => {
    const all = getAllPTEvidenceResources();
    all.forEach((r) => {
      expect(getPTEvidenceResourceById(r.id)).toEqual(r);
    });
  });

  it("should handle errors gracefully", () => {
    expect(() => {
      getPTEvidenceResourceById(null as any);
      getAllPTEvidenceResources();
      getPTEvidenceResourcesByCategory("test");
      searchPTEvidenceResources("test");
      getPTEvidenceResourcesByEvidenceLevel(1);
      getKeyFindings();
  ...
  it("should have all required properties", () => {
    ...
    all.forEach((r) => {
      ...
      expect(r.recommendedTools).toBeDefined();
  ...
  it("should have non-empty arrays", () => {
    ...
    all.forEach((r) => {
      ...
      expect(r.recommendedTools.length).toBeGreaterThan(0);
